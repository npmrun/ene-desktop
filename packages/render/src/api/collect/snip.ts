...
export async function getSnipCodes(key?: string) {
    if (key) {
        const array = (await db.collect_snipcode.where("from").equals(key).sortBy("order")) as CollectSnipCode[] ?? []
        return array
    }
    const array = (await db.collect_snipcode.orderBy("order").toArray()) as CollectSnipCode[]
    return array
}
...
export async function updateOneSnip(snip: CollectSnip, files: CollectSnipCode[], originFiles: CollectSnipCode[]) {
    return await db.transaction('rw', db.collect_snip, db.collect_snipcode, async () => {
        await db.collect_snip.update(snip.key, {
            ...
            activeCodeIndex: snip.activeCodeIndex
        })
